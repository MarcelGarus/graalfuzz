...
final class CoverageNode extends ExecutionEventNode {
    ...
    /**
     * The {@link ExecutionEventNode} class let's us define several events that
     ...
     * ).
     *
     * Each node keeps a {@link #covered} flag so that the removal only happens
     * once. The fact that the flag is annotated with
     * {@link CompilerDirectives.CompilationFinal} means that this flag will be
     * treated as {@code final} during compilation of instrumented source code
     * (i.e. the {@code false} branch of the if statement can be optimized
     * away).
     *
     * The way it's used in this method is a pattern when writing Truffle nodes:
     * <ul>
     * <li>If we are compiling a covered node, the if condition will evaluate to
     * false and the if-guarded code will be optimized away. This means that
     * once this {@link SourceSection} is confirmed to be covered, there is no
     * further instrumentation overhead on performance.
     * <li>If we are compiling a not-yet-covered node, the if condition will
     * evaluate to true, and the if-guarded code will be included for
     ...
     * execution should return to the interpreter and the existing compiled code
     * is no longer valid (since once the covered flag is set to true, the check
     * is unnecessary). The code following the directive is thus always executed
     * in the interpreter: We set the {@link #covered} flag to true, ensuring
     * that the next compilation will have no instrumentation overhead on
     * performance.</li>
     * </ul>
     *
     * @param vFrame unused
